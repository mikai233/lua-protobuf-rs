...
macro_rules! add_message_trait_method {
    ($methods:ident, $target:ty, $wrapper:ty) => {
        ...
        $methods.add_method("write_length_delimited_to_vec", |_, this, ()| {
            let mut vec = Vec::new();
            protobuf::Message::write_length_delimited_to_vec(&**this, &mut vec)
                .map_err(|e| anyhow::anyhow!(e))?;
            Ok(vec)
        });

        $methods.add_method_mut("merge_from_bytes", |_, this, bytes: Vec<u8>| {
            protobuf::Message::merge_from_bytes(&mut **this, &bytes)
                .map_err(|e| anyhow::anyhow!(e))?;
            Ok(())
        });

        $methods.add_function("parse_from_bytes", |_, bytes: Vec<u8>| {
            let myself: $target = <$target as protobuf::Message>::parse_from_bytes(&bytes)
                .map_err(|e| anyhow::anyhow!(e))?;
        ...
        $methods.add_method("write_to_bytes", |_, this, ()| {
            let bytes =
                protobuf::Message::write_to_bytes(&**this).map_err(|e| anyhow::anyhow!(e))?;
            Ok(bytes)
        });

        $methods.add_method("write_length_delimited_to_bytes", |_, this, ()| {
            let bytes = protobuf::Message::write_length_delimited_to_bytes(&**this)
                .map_err(|e| anyhow::anyhow!(e))?;
            Ok(bytes)
        });
