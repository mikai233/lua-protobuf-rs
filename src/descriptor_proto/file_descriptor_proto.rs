...
impl LuaUserData for LuaFileDescriptorProto {
    fn add_fields<F: UserDataFields<Self>>(fields: &mut F) {
        ...
        fields.add_field_method_get("package", |_, this| {
            Ok(this.package.clone())
        });

        fields.add_field_method_get("dependency", |_, this| {
            Ok(this.dependency.clone())
        });
        ...
        fields.add_field_method_get("message_type", |_, this| {
            let message_type = this.message_type.iter().map(Clone::clone).map(Into::into).collect::<Vec<LuaDescriptorProto>>();
            Ok(message_type)
        });

        fields.add_field_method_get("enum_type", |_, this| {
            let enum_type = this.enum_type.iter().map(Clone::clone).map(Into::into).collect::<Vec<LuaEnumDescriptorProto>>();
            Ok(enum_type)
        });

        fields.add_field_method_get("service", |_, this| {
            let service = this.service.iter().map(Clone::clone).map(Into::into).collect::<Vec<LuaServiceDescriptorProto>>();
            Ok(service)
        });

        fields.add_field_method_get("extension", |_, this| {
            let extension = this.extension.iter().map(Clone::clone).map(Into::into).collect::<Vec<LuaFieldDescriptorProto>>();
            Ok(extension)
        });

        fields.add_field_method_get("options", |_, this| {
            let options: Option<LuaFileOptions> = this.options.clone().into_option().map(Into::into);
            Ok(options)
        });

        fields.add_field_method_get("source_code_info", |_, this| {
            let source_code_info: Option<LuaSourceCodeInfo> = this.source_code_info.clone().into_option().map(Into::into);
            Ok(source_code_info)
        });

        fields.add_field_method_get("syntax", |_, this| {
            Ok(this.syntax.clone())
        });
    }
    ...
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method("has_package", |_, this, ()| {
            Ok(this.has_package())
        });
        ...
        methods.add_method_mut("mut_package", |_, this, ()| {
            Ok(this.mut_package().clone())
        });

        methods.add_method_mut("take_package", |_, this, ()| {
            Ok(this.take_package())
        });

        methods.add_method("syntax", |_, this, ()| {
            Ok(this.syntax().to_string())
        });

        methods.add_method("has_syntax", |_, this, ()| {
            Ok(this.has_syntax())
        });
        ...
        methods.add_method_mut("mut_syntax", |_, this, ()| {
            Ok(this.mut_syntax().clone())
        });

        methods.add_method_mut("take_syntax", |_, this, ()| {
            Ok(this.take_syntax())
        });
