...
impl LuaUserData for LuaEnumValueDescriptorProto {
    fn add_fields<F: UserDataFields<Self>>(fields: &mut F) {
        ...
        fields.add_field_method_get("options", |_, this| {
            let options: Option<LuaEnumValueOptions> = this.options.clone().into_option().map(Into::into);
            Ok(options)
        });
    }
    ...
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        add_message_trait_method!(methods, EnumValueDescriptorProto, LuaEnumValueDescriptorProto);

        add_message_dyn_trait_method!(methods, EnumValueDescriptorProto, LuaEnumValueDescriptorProto);

        add_message_full_trait_method!(methods, EnumValueDescriptorProto, LuaEnumValueDescriptorProto);
    }
}