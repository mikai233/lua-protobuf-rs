...
impl LuaUserData for LuaEnumDescriptorProto {
    fn add_fields<F: UserDataFields<Self>>(fields: &mut F) {
        ...
        fields.add_field_method_get("options", |_, this| {
            let options: Option<LuaEnumOptions> = this.options.clone().into_option().map(Into::into);
            Ok(options)
        });

        fields.add_field_method_get("reserved_range", |_, this| {
            let reserved_range = this.reserved_range.iter().map(Clone::clone).map(Into::into).collect::<Vec<LuaEnumReservedRange>>();
            Ok(reserved_range)
        });
