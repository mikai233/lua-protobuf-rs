...
impl LuaUserData for LuaDescriptorProto {
    fn add_fields<F: UserDataFields<Self>>(fields: &mut F) {
        ...
        fields.add_field_method_get("extension_range", |_, this| {
            let extension_range: Vec<LuaExtensionRange> = this
                .extension_range
                .iter()
                .map(Clone::clone)
                .map(From::from)
        ...
        fields.add_field_method_get("oneof_decl", |_, this| {
            let oneof_decl: Vec<LuaOneofDescriptorProto> = this
                .oneof_decl
                .iter()
                .map(Clone::clone)
                .map(From::from)
        ...
        fields.add_field_method_get("reserved_range", |_, this| {
            let reserved_range: Vec<LuaReservedRange> = this
                .reserved_range
                .iter()
                .map(Clone::clone)
                .map(From::from)
    ...
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method_mut("clear_name", |_, this, ()| Ok(this.clear_name()));

        methods.add_method_mut("set_name", |_, this, name: String| Ok(this.set_name(name)));
