...
impl LuaUserData for LuaMethodOptions {
    fn add_fields<F: UserDataFields<Self>>(fields: &mut F) {
        ...
        fields.add_field_method_get("idempotency_level", |_, this| {
            Ok(this.idempotency_level.as_ref().map(|e| e.enum_value_or_default() as i32))  // 可能是 Option<EnumOrUnknown>
        });
    ...
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method_mut("set_idempotency_level", |_, this, v: i32| {
            let level = IdempotencyLevel::from_i32(v).ok_or(anyhow!("unknown idempotency_level {v}"))?;
