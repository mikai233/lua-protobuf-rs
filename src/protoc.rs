    ...
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        methods.add_function(
            ...
            |_, (inputs, includes): (Vec<String>, Vec<String>)| {
                ...
                let protoc = LuaProtoc::parse_files(inputs, includes)
                    .map_err(|e| anyhow!("parse_files failed: {:?}", e).into_lua_err())?;
                Ok(protoc)
            },
        );
        methods.add_function("parse_proto", |_, proto: String| {
            let protoc = LuaProtoc::parse_proto(proto)
                .map_err(|e| anyhow!("parse_proto proto failed: {:?}", e).into_lua_err())?;
            Ok(protoc)
        });
        methods.add_function("parse_pb", |_, dir: String| {
            let protoc = LuaProtoc::parse_pb(dir)
                .map_err(|e| anyhow!("parse_pb failed: {:?}", e).into_lua_err())?;
            Ok(protoc)
        });
        methods.add_method("gen_pb", |_, this, path: String| {
            this.gen_pb(path)
                .map_err(|e| anyhow!("gen_pb failed: {:?}", e).into_lua_err())?;
            Ok(())
        });
        methods.add_method("gen_lua", |_, this, path: String| {
            this.gen_lua(path)
                .map_err(|e| anyhow!("gen_lua failed: {:?}", e).into_lua_err())?;
            Ok(())
        });
        methods.add_method(
            "encode",
            |_, protoc, (message_full_name, lua_message): (String, Table)| {
                ...
                let message = protoc
                    ...
                    .map_err(|e| anyhow!("encode message failed: {:?}", e).into_lua_err());
        ...
        methods.add_method(
            ...
            |lua, protoc, (message_full_name, message_bytes): (String, Vec<u8>)| {
                ...
                let message = protoc
                    ...
                    .map_err(|e| anyhow!("decode message failed: {:?}", e).into_lua_err());
                let message = ErrorContext::context(message, ctx)?;
                Ok(message)
            },
        );
